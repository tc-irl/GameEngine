...
void Lab2::initShaders()
{
	...
	shader->LoadFile("../Resources/Shaders/diffuse.vs",vs);
	shader->AddShader(shader->GetProgramID(), vs.c_str(), GL_VERTEX_SHADER);

	shader->LoadFile("../Resources/Shaders/diffuse.ps",ps);
	shader->AddShader(shader->GetProgramID(), ps.c_str(), GL_FRAGMENT_SHADER);
...
void Lab2::initTweakBar()
{
	...
	/*TwAddVarRW(bar, "Index", TW_TYPE_INT32, &index, 
		" label='Index Force Point' min=0 max=7");
		*/
	...
	//TwAddVarRW(bar, "Vertex[0]", TW_TYPE_DIR3F, &objectBuffer->, "group='Object Properties' label='Velocity: '");
...
void Lab2::initCube()
{
	...
	//objectBuffer->GenerateVBO(g_vertex_buffer_data,g_color_buffer_data);
